...
fun StrandraeuberLobbyScreen(
    ...
    LaunchedEffect(uid) {
        ...
        try {
            ...
            isFavorite = (snap.get("favoriteGames") as? List<String>)?.contains("strandraeuber") == true
            val prefDiff = snap.getString("preferredStrandraeuberDifficulty")
            if (!prefDiff.isNullOrBlank()) difficulty = prefDiff
            val prefRounds = snap.getLong("preferredStrandraeuberRounds")?.toInt()
            if (prefRounds != null && prefRounds in listOf(1, 3, 5)) totalRounds = prefRounds
        } catch (_: Exception) {}
    }
    ...
    fun toggleFavorite() {
        ...
        val update = if (isFavorite) FieldValue.arrayUnion("strandraeuber") else FieldValue.arrayRemove("strandraeuber")
    ...
    fun cancelWaiting() {
        if (gameDocId.isNotBlank()) {
            db.collection("strandraeuberGames").document(gameDocId).delete()
        }
        gameDocId = ""
        gameCode = ""
    ...
    Scaffold(
            ...
            TopAppBar(
                title = {
                    Column {
                        ...
                        Text("🦹 Karten ziehen & Paare ablegen", style = MaterialTheme.typography.titleLarge, color = TextPrimary, fontWeight = FontWeight.ExtraBold)
                    }
                },
                navigationIcon = {
        ...
        Column(
            ...
            if (step == "ai_config") {
                ...
                // Rounds
                Text("Rundenanzahl", style = MaterialTheme.typography.labelLarge, color = TextSub, modifier = Modifier.padding(start = 4.dp))
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (step == "online") {
                ...
                if (onlineStep == "choose") {
                    ...
                    // Rounds selection for online too
                    Text("Rundenanzahl", style = MaterialTheme.typography.labelLarge, color = TextSub, modifier = Modifier.padding(start = 4.dp))
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        listOf(1, 3, 5).forEach { n ->
                            val selected = totalRounds == n
