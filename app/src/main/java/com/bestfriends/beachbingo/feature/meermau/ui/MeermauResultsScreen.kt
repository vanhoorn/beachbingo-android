...
@Composable
fun MeermauResultsScreen(onNavigateBack: () -> Unit) {
    val uid = Firebase.auth.currentUser?.uid ?: ""
    val db = Firebase.firestore
    var results by remember { mutableStateOf<List<MeermauResultEntry>>(emptyList()) }
    val dateFormat = remember { SimpleDateFormat("dd.MM.yyyy HH:mm", Locale.GERMAN) }

    LaunchedEffect(uid) {
        if (uid.isBlank()) return@LaunchedEffect
        db.collection("meermauResults")
        ...
        if (results.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(results, key = { it.id }) { result ->
                    MeermauResultCard(result = result, currentUid = uid, dateFormat = dateFormat)
                }
            }
        }
    }
...
@Composable
private fun MeermauResultCard(
    result: MeermauResultEntry,
    currentUid: String,
